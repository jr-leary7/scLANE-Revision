...
```{python}
ad_bcell.uns['iroot'] = np.argmin(ad_bcell.obsm['X_diffmap'][:, 0])
...
We use the `scLANE` package to perform trajectory DE testing across pseudotime for a set of 3,000 candidate genes. Since our dataset is of moderate size and composed of samples from multiple subjects we'll utilize the GEE mode of `scLANE`. 
