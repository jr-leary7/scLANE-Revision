...
```{r}
traj_gene_set <- HVFInfo(seu_fish) %>% 
                 filter(mean > 0.1) %>% 
...
RNA_counts <- as.matrix(seu_fish@assays$RNA$counts)[traj_gene_set$gene, ]
ts_pt <- slingPseudotime(sling_res, na = FALSE) %>% 
         as.data.frame() %>% 
         mutate(across(contains("Lineage"), \(x) (x - min(x, na.rm = T)) / (max(x, na.rm = T) - min(x, na.rm = T))))
