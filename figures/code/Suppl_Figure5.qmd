...
non_sclANE_dyn_genes_top8 <- filter(scLANE_de_res, Gene %in% non_sclANE_dyn_genes) %>% View()
                             slice_head(n = 8) %>%
                             pull(Gene)
...
smoothed_counts <- smoothedCountsMatrix(scLANE_models, 
                                        ...
                                        log1p.norm = TRUE)
gene_embed <- embedGenes(smoothed_counts$Lineage_A, resolution.param = 0.3) %>% 
...
S_Fig_5C <- mutate(embed_df_sort, 
            ...
            mutate(program_label = case_when(program == "cluster_1" ~ "B-cell Activation", 
                                             ...
                                             program == "cluster_3" ~ "Ribosome Biogenesis", 
