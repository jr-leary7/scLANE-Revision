...
```{r}
sig_TFs_scLANE <- inner_join(scLANE_de_res, 
                             hs_tfs, 
                             by = c("Gene" = "hgnc_symbol")) %>% 
                  filter(Gene_Dynamic_Overall == 1)
sig_TFs_ts <- inner_join(ts_de_res, 
                         hs_tfs, 
                         by = c("gene" = "hgnc_symbol")) %>% 
              filter(gene_dynamic_overall == 1)
sig_TFs_lamian <- inner_join(lamian_de_res, 
                             hs_tfs, 
                             by = c("gene" = "hgnc_symbol")) %>% 
                  filter(gene_dynamic_overall == 1)
```
...
Fig_6D <- data.frame(subject_id = seu_bcell@meta.data$fetal.ids, 
          ...
          guides(color = guide_legend(ncol = 2, override.aes = list(alpha = 1, linewidth = 1.25)))
Fig_6D
```
...
scLANE_dyn_genes_only <- dyn_genes[!dyn_genes %in% unique(c(dyn_genes_ts, dyn_genes_lamian))]
non_scLANE_sig_TFs <- unique(c(sig_TFs_ts$gene, sig_TFs_lamian$gene))[!unique(c(sig_TFs_ts$gene, sig_TFs_lamian$gene)) %in% sig_TFs_scLANE$Gene]
...
```{r}
non_scLANE_enrich_res <- gprofiler2::gost(non_sclANE_dyn_genes, 
                                          organism = "hsapiens", 
                                          ordered_query = TRUE, 
                                          multi_query = FALSE, 
                                          sources = "GO:BP")$result %>% 
                         filter(p_value < 0.1)
scLANE_only_enrich_res <- gprofiler2::gost(scLANE_dyn_genes_only, 
                                           organism = "hsapiens", 
                                           ordered_query = TRUE, 
                                           multi_query = FALSE, 
                                           sources = "GO:BP")$result %>% 
...
length(dyn_genes)
length(dyn_genes_ts)
length(dyn_genes_lamian)
length(dyn_genes_ts) / 3000
length(non_sclANE_dyn_genes)
```
